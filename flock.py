# flock.py - from http://code.activestate.com/recipes/498171/
...
class flock(object):
    ...
    def acquire(self):
        ...
        if self.islocked():
            if self.debug:
                lock = self._readlock()
                print('Previous lock detected:', self.pddr(lock))
            return False
        try:
            fh = open(self.path, 'w')
            fh.write(self.addr())
    ...
    def getaddr(self):
        '''Get the pid and host of the process owning the lock'''
        lock = self._readlock()
        return (int(lock['pid']),lock['host'])
